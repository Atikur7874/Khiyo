  ...
  const handlePaymentSuccess = async (details) => {
    try {
      ...
      if (response.status === 200) {
        const order = await handleFinalizeCheckout(checkoutId);
        navigate(`/order-confirmation?orderId=${order._id}`, {
          state: { order },
        });
      } else {
        console.error("Payment request did not succeed");
      }
    } catch (paymentError) {
        ...
        <div className="border-t py-4 mb-4">
          {cart.products.map((product, index) => (
            ...
            >
              ...
              <p className="text-xl">BDT {product.price?.toLocaleString()}</p>
            </div>
          ))}
        </div>
        <div className="flex justify-between items-center text-lg mb-4">
          <p>Sub total</p>
          <p>BDT {cart.totalPrice?.toLocaleString()}</p>
        </div>
        <div className="flex justify-between items-center text-lg">
          <p>Shipping</p>
          <p>Free</p>
        </div>
        <div className="flex justify-between items-center text-lg mt-2 border-t pt-4">
          <p>Total</p>
          <p>BDT {cart.totalPrice?.toLocaleString()}</p>
        </div>
      </div>
